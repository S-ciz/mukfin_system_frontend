...
function LeaveApproval() {
  ...
  function formatMappedRequestForExcel() {
    const mapped = requests.map((item) => {
      return {
        name: item.name,
        dates: item.dates.join(" "),
        reason: item.reason,
        "manager decision": item.managerApproval,
        "HR decision": item.hrApproval,
        Status: item.status,
      };
    });
                      ...
                      {user.userType === "manager" &&
                        ...
                        req.status !== "rejected" && (
                          <div className="flex gap-2">
                            <button
                              onClick={() =>
                                handleManagerAction(req._id, "approve")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleManagerAction(req._id, "reject")
                              }
                              disabled={actionLoading === req._id}
