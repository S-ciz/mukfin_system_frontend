...
import { isInRadius } from "../services/geolocation";
import {
  createAttendance,
  updateAttendance,
  findTodayRecord,
...
function ClockForm({ onRecordUpdate }) {
  ...
  const handleClockIn = async () => {
    ...
    if (!coord) {
      ...
      setMessage({
        type: "error",
        text: `You are more than 0.3km away from the MUKFIN office in ${country}!`,
      });
    ...
    try {
      ...
      if (existingRecord) {
        if (existingRecord.clockIn) {
          ...
          setMessage({
            type: "success",
            text: "Clock-in updated successfully!",
          });
          onRecordUpdate();
        }
      } else {
  ...
  const handleClockOut = async () => {
    ...
    try {
      ...
      if (existingRecord) {
        ...
        setMessage({
          type: "warning",
          text: "Clocked out without clocking in first",
        });
        onRecordUpdate();
      }
    } catch (error) {
